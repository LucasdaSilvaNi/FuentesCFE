...
{
    public partial class frmRpt_ValidacionAnulacionCobranzaNC : Form
    {
        private TEPEntities db= new TEPEntities(Helper.CadenaConexion);

        public frmRpt_ValidacionAnulacionCobranzaNC()
        {
            InitializeComponent();
        ...
        private bool Grabar()
        {
            ...
            try
            {
                ...
                List<rpt_validacion_cobros_notascredito_anuladas_Result> lst = db.rpt_validacion_cobros_notascredito_anuladas(Helper.Oficina, _entidad, dtI, dtF, upTipo.Value.ToString()).ToList<rpt_validacion_cobros_notascredito_anuladas_Result>();
                ...
                List<rpt_validacion_cobros_notas_credito_anuladas_resumen_Result> lstA = db.rpt_validacion_cobros_notas_credito_anuladas_resumen(Helper.Oficina, _entidad, dtI, dtF, upTipo.Value.ToString()).ToList<rpt_validacion_cobros_notas_credito_anuladas_resumen_Result>();

                DataSet dsData = Helper.ToDataSet<rpt_validacion_cobros_notascredito_anuladas_Result>(lst);
                dsData.Tables[0].TableName = "pwcia";

                DataTable dt = Helper.ToDataSet<rpt_validacion_cobros_notas_credito_anuladas_resumen_Result>(lstA).Tables[0].Copy();
                ...
                frm._data = dsData;
                frm._reportName = "rpt_validacion_anulacion_cobros_notascredito.rdlc";
        ...
        private void frmRegion_Load(object sender, EventArgs e)
        {
            ...
            uccFechaInicial.Value =(new PeriodoBL()).PorId((Helper.OficinaActiva as tbl_oficina).periodo_proceso).AddMonths(-1);

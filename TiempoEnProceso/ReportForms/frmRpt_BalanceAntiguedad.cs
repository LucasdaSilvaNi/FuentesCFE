    ...
    public partial class frmRpt_BalanceAntiguedad : Form
    {
        ...
        private bool Grabar()
        {
            ...
            try
            {
                ...
                tbl_empleados _empleado = Helper.UsuarioActivo as tbl_empleados;
                string _socio = uckTodosSocio.Checked ? null : ucSocio.Value.ToString();
                string _encargado = uckTodosEncargado.Checked ? null : ucEncargado.Value.ToString();
                ...
                switch (uceAgrupamiento.Value.ToString())
                {
                    case "0":
                        report = "rpt_balance_antiguedad" + (esGT ? "_gt" : "");
                        break;
                    case "3":
                        report = "rpt_balance_antiguedad_por_ente";
                        break;
                    default:
                        report = "rpt_balance_antiguedad_socio_gerente" + (esGT ? "_gt" : "");
                        break;
                ...
                if (uceAgrupamiento.Value.ToString() == "0")
                {
                    frm._parameters.Add("Fecha", dtFecha.ToShortDateString());
                    frm._parameters.Add("Oficina", _oficina);
                }

                else if (uceAgrupamiento.Value.ToString() == "3")
                {
                    frm._parameters.Add("fecha", dtFecha.ToShortDateString());
                ...
                frm._parameters.Add("Id_socio", uckTodosSocio.Checked ? Constantes.S_TODOS : _socio);
                frm._parameters.Add("Id_encargado", uckTodosEncargado.Checked ? Constantes.S_TODOS : _encargado);
                frm._parameters.Add("Id_cliente", Constantes.S_TODOS);
                frm._parameters.Add("Ordenadopor", uceAgrupamiento.Value.ToString());
        ...
        private void frmRegion_Load(object sender, EventArgs e)
        {
            ...
            uceAgrupamiento.Value = 0;
            uckTodosSocio.Checked = true;
            uckTodosEncargado.Checked = true;
            ucSocio.Focus();
        }

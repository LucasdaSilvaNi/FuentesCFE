...
{
    public partial class frmGeneracionFacElecCR : Form
    {
        ...
        private bool GeneraArchivo(string path, string tipo, decimal documento, List<sp_text_felectronica_CR_Result> list, tbl_cuenta_corriente item)
        {
            ...
            int SeccionEnc = 0, SeccionTot = 0, SeccionAct = 0;
            bool NoImpr=false, SeccionGasto=false, seccionDetalle=false;
            bool NoVaria = list.FirstOrDefault().Codigo_Tipo_Documento == "09";
            bool NoGastos = !list.Any(d => !string.IsNullOrEmpty(d.Codigo_tipo_cargo.Trim()));
            string sDetalle = string.Empty, sTotal = string.Empty, sVar = string.Empty, sGastos = string.Empty;
            ...
            foreach (DataRow dr in ds.Tables[0].Rows)
            {
                for (int j = 0; j < ds.Tables[0].Columns.Count; j++)
                {
                    ...
                    if ("Seccion_Encabezado,seccion_Detalle,Seccion_Variacion,Seccion_Gastos,Seccion_Totales".Contains(ds.Tables[0].Columns[j].ColumnName))
                    {
                        SeccionAct++;
                        ...
                        else if (ds.Tables[0].Columns[j].ColumnName == "Seccion_Gastos")
                        {
                            if (sGastos.Length > 0)
                                sGastos = sGastos.Substring(0, sGastos.Length - 1);
                            sGastos += "\n";

                            SeccionGasto = true;
                            seccionDetalle = false;
                    ...
                    if (!NoImpr)
                    {
                        switch (SeccionAct % 5)
                        {
                            ...
                            case 3:
                                ...
                                {
                                    if (!NoVaria)
                                        sGastos += string.Format("{0}|", dr.ItemArray[j].ToString().Replace("\n", " ").Trim());
                                }
                                break;
                            case 4:
                                sGastos += string.Format("{0}|", dr.ItemArray[j].ToString().Replace("\n", " ").Trim());
            ...
            if (sDetalle.Length > 0)
            {
                string[] sDet = sDetalle.Split('\n');
                
                sDetalle = "\n";
                for (int i = 1; i < sDet.Length; i++)
                {
                    string[] sV = sDet[i].Split('~');
                    if (!sV[0].StartsWith("D||"))
                    {
                        sDetalle += sV[0] + "\n";

                        if (sV.Length > 1)
                            if (!sV[1].StartsWith("V|I|01|13|0"))
                                sDetalle += sV[1] + "\n";
                    }
                }
            ...
            if (sGastos.Length>0)
            {
                string[] sGast = sGastos.Split('\n');
                sGastos = "\n";
                foreach(string gasto in sGast) {
                    if (gasto.Length>0 &&
                        gasto!= "O|||||0.00|0.00")
                        sGastos += gasto + "\n";
                }
            ...
            sLinea += sDetalle + (!NoGastos ? sGastos : "") + sTotal;
            ...
            else
            {
                ...
                item.clave_electronica = xmlDoc.Descendants("ClaveDocumento")
                                .Single().Value;

                item.numero_documento_fiscal_electronico = xmlDoc.Descendants("NumeroDocumento")
                                .Single().Value;
                ...
                byte[] bytes = Convert.FromBase64String(xmlDoc.Descendants("XMLBase64").Single().Value);
                File.WriteAllBytes(string.Format(@"{0}\{1}.xml", path, docto), bytes);

                if (xmlDoc.Descendants("PDFBase64").SingleOrDefault() != null)
                {
                    bytes = Convert.FromBase64String(xmlDoc.Descendants("PDFBase64").Single().Value);
